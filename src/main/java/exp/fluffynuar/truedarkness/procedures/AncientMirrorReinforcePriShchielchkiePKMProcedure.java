...
public class AncientMirrorReinforcePriShchielchkiePKMProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.isShiftKeyDown()) {
			...
			if (entity instanceof Player _player)
				_player.getCooldowns().addCooldown(TruedarknessModItems.SOUL_MIRROR.get(), 40);
		} else {
			if ((entity.getCapability(TruedarknessModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new TruedarknessModVariables.PlayerVariables())).SoulCount == (world.getLevelData().getGameRules()
					.getInt(TruedarknessModGameRules.SOUL_COUNT))) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(TruedarknessModMobEffects.SOUL_SPEED.get(), (int) (1200 * 3), 1, false, false));
				{
					double _setval = 0;
