...
public class SoulstealWarriorEntity extends Monster {
	...
	public SoulstealWarriorEntity(EntityType<SoulstealWarriorEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 12;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(TruedarknessModEntities.SOULSTEAL_WARRIOR.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
