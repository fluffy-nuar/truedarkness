...
public class SculkFeatherItem extends Item {
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			ItemStack stack = ProjectileWeaponItem.getHeldProjectile(entity, e -> e.getItem() == SculkFeatherProjectileEntity.PROJECTILE_ITEM.getItem());
			if (stack == ItemStack.EMPTY) {
				for (int i = 0; i < player.getInventory().items.size(); i++) {
					ItemStack teststack = player.getInventory().items.get(i);
					if (teststack != null && teststack.getItem() == SculkFeatherProjectileEntity.PROJECTILE_ITEM.getItem()) {
						stack = teststack;
						break;
					}
				}
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				SculkFeatherProjectileEntity projectile = SculkFeatherProjectileEntity.shoot(world, entity, world.getRandom());
