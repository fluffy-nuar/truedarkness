...
public class YteriasStructure3Feature extends Feature<NoneFeatureConfiguration> {
	...
	public YteriasStructure3Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("truedarkness", "yterias_structure_3"));
		if (template == null)
			return false;
