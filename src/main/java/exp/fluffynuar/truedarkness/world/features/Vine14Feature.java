...
import exp.fluffynuar.truedarkness.procedures.Vine1NaPrimierieStrukturySghienierirovanoProcedure;

public class Vine14Feature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("truedarkness", "vine1"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.values()[context.random().nextInt(2)]).setRotation(Rotation.values()[context.random().nextInt(3)]).setRandom(context.random())
					...
					Vine1NaPrimierieStrukturySghienierirovanoProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
