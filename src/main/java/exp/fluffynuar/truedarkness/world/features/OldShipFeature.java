...
public class OldShipFeature extends Feature<NoneFeatureConfiguration> {
	...
	public OldShipFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(Blocks.WATER);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("truedarkness", "old_ship"));
		if (template == null)
			return false;
