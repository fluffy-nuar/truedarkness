...
public class RuinedPortalFeature extends Feature<NoneFeatureConfiguration> {
	...
	public RuinedPortalFeature() {
		...
		base_blocks = List.of(TruedarknessModBlocks.SAND_OF_MISSING_LAYER.get(), TruedarknessModBlocks.SWAMP_SAND.get(), TruedarknessModBlocks.SAND_OF_MISSING.get(), TruedarknessModBlocks.SWAMP_SAND_LAYER.get(),
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("truedarkness", "ruin_of_portal"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 750000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (!base_blocks.contains(context.level().getBlockState(new BlockPos(i, j, k)).getBlock()))
					continue;
				BlockPos spawnTo = new BlockPos(i + 5, j + 0, k + 2);
