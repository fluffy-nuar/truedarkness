...
public class HaltingPointFeature extends Feature<NoneFeatureConfiguration> {
	...
	public HaltingPointFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(TruedarknessModBlocks.MYCELIUM_GRASS.get(), TruedarknessModBlocks.MYCELIUM_ROOTS.get(), TruedarknessModBlocks.LIGHT_MUSHROOM_TALL.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("truedarkness", "halting_point"));
		if (template == null)
			return false;
